          ...
          <div class="websites">
                ...
                <?php
                    ...
                    if (!isset($_GET["filter"])) {
                        $sql = "SELECT Websites.Name as Name, Websites.Id AS Wid, 
                                ...
                                WHERE Parameters.buffer_two = 'germany'
                    ...
                    if (isset($_GET["filter"])) {
                        ...
                        $sql = "SELECT Websites.Name as Name, Websites.Id AS Wid, 
                                ...
                                FROM Websites 
                                JOIN Parameters on Websites.Id = Parameters.WebsiteId
                                WHERE Parameters.buffer_two = 'germany' AND
                    ...
                    while($row = mysqli_fetch_assoc($rows)) {
                        ...
                        echo "<tr>";
                        echo "<td>".$row["Name"]."</td>";
                        echo "<td><a href='activateScript.php?country=germany&id=".$row["Wid"]."&value=".$row["IsScriptActive"]."' id='execute'><span id='executeValue' class='material-icons-sharp'>$result_script</span></a></td>";
                        echo "</tr>";
                    }
                ?>
              </tbody>           
            ...
            <div class="items">
                <?php  
                    ...
                    if($page_number>=2){   
                        if(!isset($_GET["filter"])){
                          echo "<a href='germanySources.php?page=".($page_number-1)."'>  Prev </a>";   
                        }   
                        
                        if(isset($_GET["filter"])){
                          echo "<a href='germanySources.php?page=".($page_number-1)."&filter=".$_GET["filter"]."'>  Prev </a>";  
                        }
                    }
                    for ($i=1; $i<=$total_pages; $i++) {   
                        if ($i == $page_number) {   
                            if(!isset($_GET["filter"])){
                                $pageURL .= "<a class = 'activePage' href='germanySources.php?page=".$i."&filter=".$_GET["filter"]."'>".$i." </a>";   
                            }
                            ...
                            if(!isset($_GET["filter"])){
                              $pageURL .= "<a href='germanySources.php?page=".$i."'>".$i." </a>";   
                            }
                            
                            if(isset($_GET["filter"])){
                              $pageURL .= "<a href='germanySources.php?page=".$i."&filter=".$_GET["filter"]."'>".$i." </a>";   
                            }
                        }
                    }
                    echo $pageURL;  
                    if($page_number<$total_pages){   
                        if(!isset($_GET["filter"])){
                          echo "<a href='germanySources.php?page=".($page_number+1)."'>  Next </a>";   
                        }
                        
                        
                         if(isset($_GET["filter"])){
                          echo "<a href='germanySources.php?page=".($page_number+1)."&filter=".$_GET["filter"]."'>  Next </a>";   
                        }
                    }
                ?>
            </div>
    ...
    function filterWesbite() {
        ...
        if (true){
            if (filter.length > 0) {
                ...
                window.location.href = "germanySources.php";
            }
        } else {
            window.location.href = "germanySources.php";
        }

          ...
          <div class="websites">
              ...
              <tbody id="mainData">
                <?php
                    ...
                    while($row = mysqli_fetch_assoc($rows)) {
                        ...
                        echo "<tr>";
                        echo "<td>".$row["Name"]."</td>";
                        echo "<td><a href='activateScript.php?country=sweden&id=".$row["Wid"]."&value=".$row["IsScriptActive"]."' id='execute'><span id='executeValue' class='material-icons-sharp'>$result_script</span></a></td>";
                        echo "</tr>";
                    }
                ?>
              </tbody>           
            ...
            <div class="items">
                <?php  
                    ...
                    if($page_number>=2){   
                        if(!isset($_GET["filter"])){
                          echo "<a href='swedenSources.php?page=".($page_number-1)."'>  Prev </a>";   
                        }   
                        
                        if(isset($_GET["filter"])){
                          echo "<a href='swedenSources.php?page=".($page_number-1)."&filter=".$_GET["filter"]."'>  Prev </a>";  
                        }
                    }
                    for ($i=1; $i<=$total_pages; $i++) {   
                        if ($i == $page_number) {   
                            if(!isset($_GET["filter"])){
                                $pageURL .= "<a class = 'activePage' href='swedenSources.php?page=".$i."&filter=".$_GET["filter"]."'>".$i." </a>";   
                            }
                            ...
                            if(!isset($_GET["filter"])){
                              $pageURL .= "<a href='swedenSources.php?page=".$i."'>".$i." </a>";   
                            }
                            
                            if(isset($_GET["filter"])){
                              $pageURL .= "<a href='swedenSources.php?page=".$i."&filter=".$_GET["filter"]."'>".$i." </a>";   
                            }
                        }
                    }
                    echo $pageURL;  
                    if($page_number<$total_pages){   
                        if(!isset($_GET["filter"])){
                          echo "<a href='swedenSources.php?page=".($page_number+1)."'>  Next </a>";   
                        }
                        
                        
                         if(isset($_GET["filter"])){
                          echo "<a href='swedenSources.php?page=".($page_number+1)."&filter=".$_GET["filter"]."'>  Next </a>";   
                        }
                    }
                ?>
            </div>
